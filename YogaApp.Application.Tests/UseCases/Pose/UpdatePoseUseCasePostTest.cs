...
public class UpdatePoseUseCasePostTest
{
    [Fact]
    ...
    [Fact]
    public async Task ExecuteUpdatePoseToDbAsync_ThrowsException_WhenInvalidData()
    {
        //ARRANGE
        var mockPoseRepo = new Mock<IPoseRepository>();
        var mockCatRepo = new Mock<ICategoryRepository>();
        ...
        //ACT & ASSERT 
        await Assert.ThrowsAsync<ArgumentException>(() => useCase.ExecuteUpdatePoseToDbAsync(dto));
    
        //Verify repositories were never called due to validation failure
        mockPoseRepo.Verify(p => p.UpdateToDbPoseAsync(It.IsAny<Domain.Entities.Pose>()), Times.Never);
        mockCatRepo.Verify(c => c.DeleteCategoriesByPoseIdAsync(It.IsAny<int>()), Times.Never);
